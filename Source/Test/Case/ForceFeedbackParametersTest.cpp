 ...
 *   Unit tests for functionality related to force feedback parameters, especially direction
 *   vectors.
 **************************************************************************************************/
...
namespace XidiTest
{
  ...
  {
    static_assert(
        kNumAxes >= 2, "Coordinate conversion tests are only valid with at least 2 axes.");

    /// Cartesian coordinates, one coordinate per element and one coordinate per axis.
    std::array<TEffectValue, kNumAxes> cartesian = {};

    /// Optional polar coordinates, either one angle value is present or it is not.
    std::optional<TEffectValue> polar = std::nullopt;

    /// Spherical coordinates, one coordinate per element and one less total number of coordinates
    /// than the number of axes.
    std::array<TEffectValue, kNumAxes - 1> spherical = {};
  };
  ...
  template <int kNumAxes> struct SMagnitudeComponentsTestData
  {
    ...
    /// Direction expressed as Cartesian coordinates.
    std::array<TEffectValue, kNumAxes> directionCartesian = {};

    /// Associated magnitude components.
  ...
  /// Otherwise the ratio is computed and it must show a difference of at most 3% for the two values
  ...
  template <typename ValueType> static bool ApproximatelyEqual(ValueType valueA, ValueType valueB)
  {
    ...
    if (((ValueType)0 == valueA) || ((ValueType)0 == valueB)) return (valueA == valueB);

    // Since we might be dealing with floating point quantities, there could be some imprecision, so
    // allow a maximum error as specified at the top of this function. A difference in sign would
    // produce a negative ratio, which would return false.
    const double ratioSimilarity = (double)valueA / (double)valueB;
    return (
        ratioSimilarity >= (1.0 - kMaxRelativeError) &&
        ratioSimilarity <= (1.0 + kMaxRelativeError));
  }
  ...
  /// @return `true` if the two magnitude component vectors all have components that are
  /// approximately equal, `false` otherwise.
  template <> static bool ApproximatelyEqual<TMagnitudeComponents>(
      TMagnitudeComponents valueA, TMagnitudeComponents valueB)
  {
    for (size_t i = 0; i < valueA.size(); ++i)
    {
      if (false == ApproximatelyEqual(valueA[i], valueB[i])) return false;
    }
  ...
  static void CheckCartesianDirectionEquivalence(
      const TEffectValue* coordinatesA, const TEffectValue* coordinatesB, int numCoordinates)
  {
    std::optional<double> maybeExpectedRatio;

    // All non-zero coordinates need to follow the same ratio and all zero coordinates need to be
    // exactly equal. We don't know what that ratio is, however, until we start comparing individual
  ...
  /// Creates a direction vector and verifies that it performs correct coordinate system conversion
  /// according to the supplied test data record.
  /// @param [in] testData Test data record to use as the basis for the test.
  /// @tparam kNumAxes Number of axes in the direction vector represented by the test data.
  template <int kNumAxes> static void DirectionVectorCoordinateConversionTest(
      const SCoordinateConversionTestData<kNumAxes>& testData)
  {
    // Conversion from Cartesian
    DirectionVector vectorCartesian;
    TEST_ASSERT(
        true ==
        vectorCartesian.SetDirectionUsingCartesian(
            &testData.cartesian[0], (int)testData.cartesian.size()));
    ...
    TEST_ASSERT(
        (int)actualCartesianToSpherical.size() ==
        vectorCartesian.GetSphericalCoordinates(
            &actualCartesianToSpherical[0], (int)actualCartesianToSpherical.size()));
    ...
    if (testData.polar.has_value())
    {
      ...
      TEST_ASSERT(
          (int)actualPolarToCartesian.size() ==
          vectorPolar.GetCartesianCoordinates(
              &actualPolarToCartesian[0], (int)actualPolarToCartesian.size()));
      CheckCartesianDirectionEquivalence(
          &expectedPolarToCartesian[0], &actualPolarToCartesian[0], (int)kNumAxes);

      const auto expectedPolarToSpherical = testData.spherical;
      decltype(testData.spherical) actualPolarToSpherical;
      TEST_ASSERT(
          (int)actualPolarToSpherical.size() ==
          vectorPolar.GetSphericalCoordinates(
              &actualPolarToSpherical[0], (int)actualPolarToSpherical.size()));
    ...
    TEST_ASSERT(
        true ==
        vectorSpherical.SetDirectionUsingSpherical(
            &testData.spherical[0], (int)testData.spherical.size()));

    const auto expectedSphericalToCartesian = testData.cartesian;
    decltype(testData.cartesian) actualSphericalToCartesian;
    TEST_ASSERT(
        (int)actualSphericalToCartesian.size() ==
        vectorSpherical.GetCartesianCoordinates(
            &actualSphericalToCartesian[0], (int)actualSphericalToCartesian.size()));
    CheckCartesianDirectionEquivalence(
        &expectedSphericalToCartesian[0], &actualSphericalToCartesian[0], (int)kNumAxes);
  ...
  /// Creates a direction vector and verifies that it performs correct magnitude component
  /// computations according to the supplied test data record.
  /// @param [in] magnitude Magnitude of the force vector for which components are desired.
  /// @param [in] testData Test data record to use as the basis for the test.
  /// @tparam kNumAxes Number of axes in the direction vector represented by the test data.
  template <int kNumAxes> static void DirectionVectorMagnitudeComponentsTest(
      TEffectValue magnitude, const SMagnitudeComponentsTestData<kNumAxes>& testData)
  {
    DirectionVector vector;
    TEST_ASSERT(
        true ==
        vector.SetDirectionUsingCartesian(
            &testData.directionCartesian[0], (int)testData.directionCartesian.size()));

    const TMagnitudeComponents& expectedMagnitudeComponents = testData.magnitudeComponents;
    const TMagnitudeComponents actualMagnitudeComponents =
        vector.ComputeMagnitudeComponents(magnitude);
    TEST_ASSERT(true == ApproximatelyEqual(actualMagnitudeComponents, expectedMagnitudeComponents));
  }

  // Exercises coordinate system setting, getting, and converting with single-axis direction
  // vectors. The only possible input coordinate system is Cartesian, and all of these attempted
  // conversions should fail.
  TEST_CASE(ForceFeedbackParameters_DirectionVector_1D_Conversions)
  {
    constexpr TEffectValue kTestCoordinates[] = {
        -100000000, -10000, -100, -1, 1, 100, 10000, 100000000};
    ...
    for (const auto testCoordinate : kTestCoordinates)
    {
      ...
      TEST_ASSERT(
          1 ==
          vector.GetCartesianCoordinates(
              &actualOutputCoordinates[0], (int)actualOutputCoordinates.size()));
      TEST_ASSERT(actualOutputCoordinates[0] == testCoordinate);

      // All conversions should fail, so there should be no output written to the actual output
      // coordinate variable.
      constexpr std::array<TEffectValue, kEffectAxesMaximumNumber> kExpectedOutputCoordinates = {
          55, 66};

      actualOutputCoordinates = kExpectedOutputCoordinates;
      TEST_ASSERT(
          0 ==
          vector.GetPolarCoordinates(
              &actualOutputCoordinates[0], (int)actualOutputCoordinates.size()));
      ...
      TEST_ASSERT(
          0 ==
          vector.GetSphericalCoordinates(
              &actualOutputCoordinates[0], (int)actualOutputCoordinates.size()));
      TEST_ASSERT(actualOutputCoordinates == kExpectedOutputCoordinates);
    }
  }
  ...
  // Magnitude of the direction vector itself does not matter, only the sign does, so the expected
  // output magnitude is single-component with the same absolute value and either the same sign of,
  // or opposite sign of, the input.
  TEST_CASE(ForceFeedbackParameters_DirectionVector_1D_MagnitudeComponents)
  {
    ...
    {
      // Verify that a zero-coordinate spherical specification works and also produces the same
      // single axis vector as above.
      {
        DirectionVector vector;
      ...
      for (const auto testCoordinate : kTestCoordinates)
      {
        ...
        const TMagnitudeComponents expectedOutput = {
            ((testCoordinate > 0) ? testMagnitude : -testMagnitude)};
        const TMagnitudeComponents actualOutput = vector.ComputeMagnitudeComponents(testMagnitude);
        TEST_ASSERT(actualOutput == expectedOutput);
      }
    }
  ...
  TEST_CASE(ForceFeedbackParameters_DirectionVector_InvalidCoordinates)
  {
    ...
    // First we use some valid angle values but with an invalid number of coordinates (the only
    // allowed number is 1). Then we use invalid angles that are either negative or out of range.
    ...
    // First we use some valid angle values but with an invalid number of coordinates (the allowed
    // range is 0 to one less than maximum allowed axes). Then we use some invalid angles that are
    // either negative or out of range.
    ...
    // Finally, verify that the vector reports not having any direction set, since all of the above
    // attempts should have failed.
    TEST_ASSERT(false == vector.HasDirection());
  }

  // Verifies that direction vector objects correctly report their original coordinate system once a
  // direction is set.
  TEST_CASE(ForceFeedbackParameters_DirectionVector_OriginalCoordinateSystem)
  {
    ...
    TEST_ASSERT(
        true == vector.SetDirectionUsingCartesian(kTestCoordinates, _countof(kTestCoordinates)));
    TEST_ASSERT(ECoordinateSystem::Cartesian == vector.GetOriginalCoordinateSystem());

    TEST_ASSERT(
        true == vector.SetDirectionUsingPolar(kTestCoordinates, _countof(kTestCoordinates)));
    TEST_ASSERT(ECoordinateSystem::Polar == vector.GetOriginalCoordinateSystem());

    TEST_ASSERT(
        true == vector.SetDirectionUsingSpherical(kTestCoordinates, _countof(kTestCoordinates)));
    TEST_ASSERT(ECoordinateSystem::Spherical == vector.GetOriginalCoordinateSystem());
  }

  // Verifies that direction vector objects correctly enter omnidirectional mode and, when in this
  // mode, broadcast force components without transformation.
  TEST_CASE(ForceFeedbackParameters_DirectionVector_Omnidirectional)
  {
    ...
    const TMagnitudeComponents expectedMagnitudeComponents = {
        kTestMagnitude, kTestMagnitude, kTestMagnitude};
    const TMagnitudeComponents actualMagnitudeComponents =
        vector.ComputeMagnitudeComponents(kTestMagnitude);
    TEST_ASSERT(actualMagnitudeComponents == expectedMagnitudeComponents);
  }

  // Verifies that direction vector objects exit omnidirectional mode once the direction is changed
  // to something else.
  TEST_CASE(ForceFeedbackParameters_DirectionVector_OmnidirectionalExit)
  {
    ...
    TEST_ASSERT(
        true ==
        vector.SetDirectionUsingCartesian(
            kTestCoordinatesOmnidirectional, _countof(kTestCoordinatesOmnidirectional)));
    TEST_ASSERT(true == vector.HasDirection());
    TEST_ASSERT(true == vector.IsOmnidirectional());
    ...
    TEST_ASSERT(
        true ==
        vector.SetDirectionUsingCartesian(
            kTestCoordinatesUnidirectional, _countof(kTestCoordinatesUnidirectional)));
    TEST_ASSERT(true == vector.HasDirection());
