...
namespace XidiTest
{
  ...
  {
    constexpr SRampForceParameters kTestMagnitudes = {
        .magnitudeStart = 0, .magnitudeEnd = kTestEffectDuration};
  ...
  // Creates a ramp force effect with constant positive magnitude and applies an envelope
  ...
  TEST_CASE(RampForceEffect_ComputeMagnitude_EnvelopePositive)
  {
    ...
    effect.SetTypeSpecificParameters(
        {.magnitudeStart = kTestMagnitude, .magnitudeEnd = kTestMagnitude});
  ...
  // Creates a ramp force effect with constant negative magnitude and applies an envelope
  // transformation. No other properties are specified.
