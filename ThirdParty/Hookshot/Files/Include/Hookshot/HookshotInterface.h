 ...
 ***********************************************************************************************//**
 * @file HookshotInterface.h
 *   Declaration of the IHookshot family of interfaces and associated functions. External users 
 *   should include Hookshot.h instead of this file.
 **************************************************************************************************/
...
namespace Hookshot
{
  ...
  public:
    ...
    static constexpr unsigned int kHookshotInterfaceVersion = 1;
  ...
  /// Version 2 of the main Hookshot interface.
  class IHookshot2 : public IHookshot
  {
  public:

    static constexpr unsigned int kHookshotInterfaceVersion = 2;
  ...
  /// Type alias for the latest available version of the Hookshot interface.
  using IHookshotLatest = IHookshot2;

  /// Concept for identifying all types that are Hookshot interface versions.
  template <typename T> concept IsHookshotInterface = std::is_base_of_v<IHookshot, T>;

  /// Safely checks if a newer Hookshot interface version is supported by the loaded library and, if
  /// so, returns a properly-typed pointer to the new version.
  /// @tparam HookshotInterface Desired Hookshot interface type.
  /// @param [in] hookshot Original Hookshot interface type received from the library. This is
  /// always going to be version 1 of the interface.
  /// @return A properly-typed pointer to the requested newer version of the Hookshot interface, if
  /// it is supported, or `nullptr` if it is not supported.
  template <typename HookshotInterface>
    requires IsHookshotInterface<HookshotInterface>
  inline HookshotInterface* RequestNewerHookshotInterface(IHookshot* hookshot)
  {
#ifdef _WIN64
    static constexpr char kGetLatestInterfaceVersionProcName[] =
        "HookshotGetLatestInterfaceVersion";
#else
    static constexpr char kGetLatestInterfaceVersionProcName[] =
        "@HookshotGetLatestInterfaceVersion@0";
#endif

    HMODULE hookshotLibraryHandle{};
    if (0 ==
        GetModuleHandleExW(
            GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT,
            reinterpret_cast<LPCWSTR>(hookshot),
            &hookshotLibraryHandle))
      return nullptr;

    // Hookshot libraries with interface version 1 do not export this version checking function at
    // all, so its absence is an automatic indication that the latest supported version is 1.
    unsigned int latestSupportedInterfaceVersion = IHookshot::kHookshotInterfaceVersion;
    auto hookshotLatestInterfaceVersionFunc = reinterpret_cast<unsigned int(__fastcall*)(void)>(
        GetProcAddress(hookshotLibraryHandle, kGetLatestInterfaceVersionProcName));
    if (nullptr != hookshotLatestInterfaceVersionFunc)
      latestSupportedInterfaceVersion = hookshotLatestInterfaceVersionFunc();

    return (
        (HookshotInterface::kHookshotInterfaceVersion <= latestSupportedInterfaceVersion)
            ? static_cast<HookshotInterface*>(hookshot)
            : nullptr);
  }
} // namespace Hookshot
